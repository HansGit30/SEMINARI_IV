...
export const Postura: React.FC = () => {
  ...
  // Bucle de predicción continua y dibujo en tiempo real
  const predictWebcamFrame = async () => {
    ...
    // Sincronizar dimensiones del canvas con las del video
    if (canvas.width !== video.videoWidth || canvas.height !== video.videoHeight) {
      canvas.width = video.videoWidth;
      canvas.height = video.videoHeight;
    }

    // Estimar postura y predecir
    ...
    if (ctx) {
      // Limpiar canvas transparente en cada frame
      ctx.clearRect(0, 0, canvas.width, canvas.height);

      if (pose && pose.keypoints) {
        const minPartConfidence = 0.4;
        // Dibujar esqueleto (líneas verdes/azules)
        tmPose.drawSkeleton(pose.keypoints, minPartConfidence, ctx, 4, "#00ffcc");
        // Dibujar puntos clave (rojos con borde blanco)
        tmPose.drawKeypoints(pose.keypoints, minPartConfidence, ctx, 6, "#ff0055", "#ffffff");
      }
    }
  ...
  const capturePhoto = () => {
    if (!videoRef.current || !canvasRef.current || livePredictions.length === 0) return;

    const video = videoRef.current;
    const keypointCanvas = canvasRef.current;

    // Crear un canvas temporal para combinar la imagen del video + los puntos dibujados
    ...
    if (tempCtx) {
      ...
      tempCtx.drawImage(keypointCanvas, 0, 0);
    }
  ...
  return (
    <div style={styles.container}>
      ...
      {/* Visor de Cámara en vivo con capas superpuestas */}
      {isCameraActive && (
        <div style={styles.cameraBox}>
          <div style={styles.videoWrapper}>
            {/* 1. Capa de Video limpia de la cámara */}
            <video
              ref={videoRef}
              autoPlay
              playsInline
            ...
            />
            {/* 2. Capa transparente Canvas superpuesta para dibujar los puntos encima */}
            <canvas ref={canvasRef} style={styles.canvasOverlay} />
          </div>

          {/* Porcentajes y predicciones en vivo */}
      ...
      {/* Controles de carga y activación */}
      {!isCameraActive && (
      ...
      {/* Historial de imágenes capturadas / subidas */}
...
// Se mantienen todos tus estilos UI originales y se añade la superposición de video/canvas
